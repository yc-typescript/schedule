import { Schedule, IRules, IDateRule, IWeeklyRule, IDailyRule, IRuleItem } from '../src/schedule';
...
test('Should schedule with daily rule', () => {
  ...
  const calendarDate = schedule.presentCurrentDate();
  expect(calendarDate.hours.map(x => x.available)).toEqual(Array(24).fill(true));
