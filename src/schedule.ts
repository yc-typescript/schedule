...
import { Calendar, ICalendarItem, CalendarDate, ICalendarHour } from './calendar';
...
export class Schedule {
  ...
  presentMonth(date: Date | moment.Moment | string) {
    ...
    const prevItems: ICalendarItem[] = Array(prevDates).fill(0)
      .map((x, y) => x + y)
      ...
      .reverse();
    const currentItems: ICalendarItem[] = Array(currentDates).fill(1)
      .map((x, y) => x + y)
      .map((x: number): ICalendarItem => {
        return {
          date: moment(m).date(x),
    ...
    const nextItems: ICalendarItem[] = Array(nextDates).fill(1)
      .map((x, y) => x + y)
      .map((x: number): ICalendarItem => {
        return {
    ...
    const items: ICalendarItem[] = [].concat(prevItems, currentItems, nextItems);
    if (this.rules.daily) {
    ...
    if (this.rules.weekly) {
      ...
      for (const week of this.rules.weekly) {
        ...
        Array(6).fill(index).map((x, y) => x + y * 7)
          .map(x => items[x])
  ...
  presentDate(date: Date | moment.Moment | string) {
    const m = moment(date).hour(0).minute(0).second(0).millisecond(0);
    const hours: ICalendarHour[] = Array(24).fill({
      available: false,
      selected: false
    });
    if (this.rules.daily) {
